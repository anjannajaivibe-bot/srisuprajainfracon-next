...
export default function AdminClient() {
  ...
  const fetchLeads = async () => {
    try {
      ...
      if (data.success) {
        const nextLeads: Lead[] = data.leads || [];
        setLeads(nextLeads);
  ...
  const saveNote = async (lead: Lead) => {
    ...
    setNoteSaveState((current) => ({ ...current, [lead.id]: "error" }));
  };
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      month: leads.filter((lead) => new Date(lead.created_at) >= monthAgo).length,
  ...
  const getStatusClassName = (status: string) => {
    ...
    if (status === "Contacted") return "border-orange-200 bg-orange-50 text-orange-700";
    if (status === "Site Visit Planned") return "border-purple-200 bg-purple-50 text-purple-700";
    if (status === "Follow-up") return "border-amber-200 bg-amber-50 text-amber-700";
    if (status === "Converted") return "border-green-200 bg-green-50 text-green-700";
    if (status === "Not Interested") return "border-red-200 bg-red-50 text-red-700";
    ...
    <div className="min-h-screen bg-slate-50 px-4 py-6 sm:px-6">
      <div className="w-full px-6 py-8">
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Supraja Infracon CRM</h1>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="rounded-2xl border border-[#C9A227]/30 bg-white px-5 py-3 shadow-sm">
              ...
              <p className="text-xs font-medium capitalize text-[#C9A227]">{userRole}</p>
            </div>

            {userRole === "admin" && (
              <>
                <a href="/admin/clicks" className="rounded-xl border border-[#C9A227]/50 bg-white px-5 py-3 text-center font-semibold text-[#0B1633]">
                  Website Clicks
                </a>
                <button onClick={exportCSV} className="rounded-xl bg-[#C9A227] px-5 py-3 font-semibold text-[#0B1633]">
                  Export CSV
                </button>
              </>
            )}
        ...
        {(stats.dueToday > 0 || stats.overdue > 0) && (
          <div className="mb-6 rounded-3xl border border-amber-200 bg-amber-50 p-5 text-amber-900">
            <strong>Follow-up Alert:</strong> {stats.dueToday} due today and {stats.overdue} overdue.
          </div>
        )}
        ...
        <div className="mb-6 grid gap-4 rounded-3xl bg-white p-5 shadow md:grid-cols-4">
          ...
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-[#C9A227]">
            <option value="All">All Statuses</option>
            {statuses.map((status) => <option key={status} value={status}>{status}</option>)}
          </select>

          <select value={projectFilter} onChange={(e) => setProjectFilter(e.target.value)} className="rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-[#C9A227]">
            <option value="All">All Projects</option>
            {projects.map((project) => <option key={project} value={project}>{project}</option>)}
          </select>

          {userRole === "admin" && (
            <select value={assigneeFilter} onChange={(e) => setAssigneeFilter(e.target.value)} className="rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-[#C9A227]">
              <option value="All">All Assignees</option>
              {salesTeam.map((person) => <option key={person} value={person}>{person}</option>)}
            </select>
          )}
        </div>
        ...
        <div className="overflow-hidden rounded-3xl bg-white shadow">
          ...
          {loading ? (
            ...
            <div className="max-h-[72vh] w-full overflow-auto">
              <table className="w-full min-w-[1800px]">
                <thead className="sticky top-0 z-20 bg-slate-100">
                  <tr>
                    ...
                    <th className="sticky right-0 z-30 bg-slate-100 p-4 text-left">Actions</th>
                    ...
                    return (
                      <tr key={lead.id} className="border-t align-top">
                        <td className="p-4">
                          ...
                          {lead.duplicate_of && <span className="mt-2 inline-block rounded-full bg-red-100 px-3 py-1 text-xs font-bold text-red-700">Duplicate Lead</span>}
                          {lead.message && <p className="mt-2 max-w-xs text-sm text-slate-500">{lead.message}</p>}
                        </td>
                        ...
                        <td className="p-4">
                          {userRole === "admin" ? (
                            ...
                            <span className="rounded-full bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-700">{lead.assigned_to || "Unassigned"}</span>
                          )}
                        </td>
                        ...
                        <td className="p-4">
                          <select
                            ...
                            className={`rounded-full border px-3 py-2 text-sm font-semibold outline-none ${getStatusClassName(lead.status)}`}
                          >
                            {statuses.map((status) => <option key={status} value={status}>{status}</option>)}
                          </select>
                        </td>
                        ...
                        <td className="p-4">
                          <div className="w-72">
                            ...
                            <div className="mt-2 flex items-center gap-2">
                              ...
                              {saveState === "saved" && <span className="text-xs font-semibold text-green-700">Saved</span>}
                              {saveState === "error" && <span className="text-xs font-semibold text-red-700">Save failed</span>}
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="p-4">
                          <div className="flex flex-col gap-2">
                            {whatsappTemplates.map((template) => {
                              const url = buildWhatsappUrl(lead.phone, template.text);
                              return (
                                <a
                                  key={template.label}
                                  href={url}
                                  target="_blank"
                                  rel="noreferrer"
                                  onClick={(event) => handleContactClick(event, lead.id, "WhatsApp", url, true)}
                        ...
                        <td className="sticky right-0 z-20 bg-white p-4">
                          <div className="flex flex-col gap-2">
                            <a href={callUrl} onClick={(event) => handleContactClick(event, lead.id, "Call", callUrl, false)} className="rounded-full bg-slate-900 px-4 py-2 text-center text-sm font-semibold text-white">
                              Call
                            </a>
                            <a href={whatsappUrl} target="_blank" rel="noreferrer" onClick={(event) => handleContactClick(event, lead.id, "WhatsApp", whatsappUrl, true)} className="rounded-full bg-green-600 px-4 py-2 text-center text-sm font-semibold text-white">
                              WhatsApp
                            </a>
                          </div>
                        </td>
                        ...
                        <td className="p-4">
                          <button
                            onClick={() => setOpenTimeline(openTimeline === lead.id ? null : lead.id)}
              ...
              {filteredLeads.length === 0 && <div className="p-10 text-center text-slate-500">No leads available.</div>}
            </div>
          )}
        </div>
      </div>
