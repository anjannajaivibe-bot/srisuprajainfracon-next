    ...
    <main className="overflow-hidden bg-white text-slate-900">
        ...
        <div className="container-max grid items-center gap-8 px-4 py-10 sm:px-6 sm:py-12 lg:grid-cols-[0.82fr_1.18fr] lg:px-8 lg:py-14">
          <div className="relative z-10 max-w-xl">
            ...
            <div className="mt-7 flex flex-col gap-3 sm:flex-row">
              <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 rounded-xl bg-green-500 px-6 py-3.5 text-sm font-bold text-white transition hover:bg-green-600"><MessageCircle size={18} /> Join as Channel Partner</a>
              <a href={`tel:${PHONE_NUMBER}`} className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/35 bg-white/5 px-6 py-3.5 text-sm font-bold text-white transition hover:bg-white/10"><Phone size={17} /> {PHONE_DISPLAY}</a>
            </div>
      ...
      <section className="border-b border-slate-100 bg-white py-11 sm:py-14">
        <div className="container-max px-4 sm:px-6 lg:px-8">
          <div className="text-center"><p className="text-xs font-bold uppercase tracking-[0.18em] text-amber-700">Why Join Us</p><h2 className="mt-2 font-display text-3xl font-bold tracking-tight text-slate-950 sm:text-4xl">Simple support for your business</h2></div>
          <div className="mt-8 grid gap-7 md:grid-cols-3">
            {benefits.map(({ icon: Icon, title, text }) => <article key={title} className="text-center md:border-r md:border-slate-200 md:last:border-r-0 md:px-7"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-amber-50 text-amber-700"><Icon size={22} /></div><h3 className="mt-4 text-base font-bold text-slate-950">{title}</h3><p className="mx-auto mt-2 max-w-xs text-sm leading-6 text-slate-600">{text}</p></article>)}
          </div>
        </div>
      </section>
      ...
      <section className="bg-slate-50 py-11 sm:py-14">
        <div className="container-max grid gap-8 px-4 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:items-center lg:px-8">
          <div className="relative aspect-[16/10] overflow-hidden rounded-xl bg-slate-200 shadow-sm"><Image src="/uploads/blog/architect-planning-home-layout.webp" alt="Real estate professionals discussing a property plan" fill sizes="(max-width: 1024px) 100vw, 48vw" className="object-cover" /></div>
          <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-amber-700">Who Can Join</p><h2 className="mt-2 max-w-xl font-display text-3xl font-bold tracking-tight text-slate-950 sm:text-4xl">If you work in property sales or referrals, speak with us</h2><p className="mt-4 max-w-xl text-base leading-7 text-slate-600">We welcome people who understand their local market, communicate clearly and can connect interested customers with suitable projects.</p><div className="mt-5 space-y-2.5">{whoCanJoin.map((item) => <div key={item} className="flex items-start gap-3"><CheckCircle2 className="mt-0.5 shrink-0 text-amber-700" size={18} /><span className="text-sm font-semibold leading-6 text-slate-700">{item}</span></div>)}</div></div>
        </div>
      </section>
