...
export default function CareersPage() {
  return (
    <main className="overflow-hidden bg-white text-slate-900">
      <section className="relative overflow-hidden bg-[#06142b] text-white">
        <div className="container-max grid items-center gap-8 px-4 py-10 sm:px-6 sm:py-12 lg:grid-cols-[0.82fr_1.18fr] lg:px-8 lg:py-14">
          ...
          <div className="relative min-h-[300px] overflow-hidden rounded-2xl lg:min-h-[390px]">
            ...
            <div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-[#06142b]/55 to-transparent" />
          </div>
        </div>
      </section>

      <section className="border-b border-slate-100 bg-white py-11 sm:py-13">
      ...
      <section className="bg-slate-50 py-11 sm:py-13">
        <div className="container-max grid gap-8 px-4 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:items-center lg:px-8">
          <div className="relative aspect-[16/10] overflow-hidden rounded-xl bg-slate-200 shadow-sm">
            <Image src="/uploads/blog/architect-planning-home-layout.webp" alt="Real estate professionals discussing a property plan" fill sizes="(max-width: 1024px) 100vw, 48vw" className="object-cover" />
          </div>
          <div>
            ...
            <div className="mt-5 space-y-2.5">
              {whoCanJoin.map((item) => (
                <div key={item} className="flex items-start gap-3"><CheckCircle2 className="mt-0.5 shrink-0 text-amber-700" size={18} /><span className="text-sm font-semibold leading-6 text-slate-700">{item}</span></div>
              ))}
            </div>
          </div>
        </div>
      ...
      <section className="py-11 sm:py-13">
        <div className="container-max grid overflow-hidden rounded-xl border border-slate-200 bg-[#fffaf2] lg:grid-cols-[1fr_1fr] lg:items-stretch">
          ...
          <div className="relative min-h-[250px] lg:min-h-[315px]">
            <Image src="/uploads/blog/architect-planning-home-layout.webp" alt="Real estate project layout discussion" fill sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover object-center" />
          </div>
        </div>
      </section>

      <section className="bg-slate-950 px-4 py-4 text-white sm:px-6">
        <div className="container-max flex items-start gap-3 text-xs leading-5 text-slate-300 sm:text-sm"><Handshake className="mt-0.5 shrink-0 text-amber-300" size={18} /><p>Channel partner opportunities are independent business arrangements and are not salaried employment. Terms are discussed and agreed separately with each partner.</p></div>
      </section>
    </main>
  );
}
